...
test('전송 권한이 없으면 어떤 API도 호출하지 않는다', async () => {
  ...
  assert.equal(calls.dm, 0);    // DM으로 우회하지 않는다
});

// 조용히 넘어가면 깨진 채널 권한을 영원히 모른다.
test('채널 전송 불가 시 빠진 권한을 로그로 남긴다', async () => {
  const { msg } = makeMsg(VIEW_ONLY);
  const lines = await captureWarn(() => safeReply(msg, 'hi'));

  assert.equal(lines.length, 1);
  const [label, detail] = lines[0];
  assert.match(label, /채널 전송 불가/);
...
test('50013이면 우회하지 않고 실패로 끝낸다', async () => {
...
test('채널이 null이어도 던지지 않는다', async () => {
...
test('10003이면 채널을 전송 대상에서 영구 제외한다', async () => {
  unreachable.clear();
  try {
