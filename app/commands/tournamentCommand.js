...
/**
 * 토너먼트코드 명령어 (TRC-226)
 * - !내전생성 [판수]   : 코드 선발급(gameType=1 일반내전) 후 실행 채널에 첫 코드 게시
 * - !스크림생성 [판수] : gameType=2 스크림으로 발급
 * - !대회생성 [판수]   : gameType=3 대회로 발급
 * - !다음코드          : 미사용 다음 코드 조회 후 게시
 *
 * gameType은 백엔드 tournament_code.game_type에 저장되고, 리플 적재 시
 * custom_match.game_type으로 전파된다 (1=일반내전/2=스크림/3=대회).
 */
...
/**
 * @description 발급 명령 공통 로직 — 유형(gameType)별로 이름·문구만 다르다.
 */
const createIssueCommand = ({ name, gameType, label }) => ({
	name,
	run: async (client, msg, args) => {
		...
		try {
			...
			const result = await tournamentClient.post_codes({ guildId, channelId, count, gameType });
...
module.exports = [
	createIssueCommand({ name: '내전생성', gameType: '1', label: '내전' }),
	createIssueCommand({ name: '스크림생성', gameType: '2', label: '스크림' }),
	createIssueCommand({ name: '대회생성', gameType: '3', label: '대회' }),
	{
		name: '다음코드',
		run: async (client, msg, args) => {
			try {
				...
				if (!code || !code.code) {
					return safeReply(msg, ':warning: 발급된 코드가 없습니다. `!내전생성`으로 새 코드를 발급하세요.');
				}
