...
- [x] 4. E2E: 발급→게시→(시뮬 콜백)→다음 코드 자동 게시 (2026-07-07 로컬 실측)
      ✅ `!내전시작` → stub 코드 3개 발급·첫 코드 채널 게시, `!다음코드` 수동 폴백
      ✅ 백엔드→봇 자동 게시: /post-next-code 시뮬 → 채널에 "다음 코드" 게시 확인
      ✅ 위조 방어: 틀린 시크릿 404
      ⏳ 실경기 적재가 자동 게시를 트리거하는 완전판은 백엔드 단계 8(실경기)과 함께
      ℹ️ 동작 설명: `!다음코드`는 "미사용(PENDING) 첫 코드"를 반환하므로, 직전 코드의
      경기가 아직 기록되지 않았으면 같은 코드가 다시 나온다 — 기록(COMPLETED) 후 다음 코드로 넘어감.
